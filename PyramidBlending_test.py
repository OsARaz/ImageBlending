...
def run_blend(black_image, white_image, mask):
  ...
  gauss_pyr_mask = PyramidBlending.gaussPyramid(mask, depth)
  gauss_pyr_black = PyramidBlending.gaussPyramid(black_image, depth)
  gauss_pyr_white = PyramidBlending.gaussPyramid(white_image, depth)


  lapl_pyr_black  = PyramidBlending.laplPyramid(gauss_pyr_black)
  lapl_pyr_white = PyramidBlending.laplPyramid(gauss_pyr_white)

  outpyr = PyramidBlending.blend(lapl_pyr_white, lapl_pyr_black, gauss_pyr_mask)
  outimg = PyramidBlending.collapse(outpyr)
...
def test_reduce_expand():
  ...
  expandin = [np.array([[255.0]]),
              np.array([[125.0, 255.0],
                        [255.0,   0.0]]),
              np.array([[ 255.0,    0.0,  125.0,  125.0,  125.0],
                        [ 255.0,    0.0,  125.0,  125.0,  125.0],
  ...
  for imgin, true_out in zip(expandin, expandout):
    ...
    usr_out = PyramidBlending.expand(imgin)
...
def test_gaussian_laplacian():
  ...
  for pyr in gauss_pyr1, gauss_pyr2:
    ...
    usr_out = PyramidBlending.gaussPyramid(pyr[0], 3)
  ...
  for gauss_pyr, lapl_pyr in zip((gauss_pyr1, gauss_pyr2), (lapl_pyr1, lapl_pyr2)):
    ...
    usr_out = PyramidBlending.laplPyramid(gauss_pyr)
...
def test_blend_collapse():
  ...
  for left_pyr, right_pyr, mask_pyr, out_pyr in ((lapl_pyr11, lapl_pyr12, mask_pyr1, out_pyr1), 
    ...
    usr_out = PyramidBlending.blend(left_pyr, right_pyr, mask_pyr)
  ...
  for pyr, img in ((out_pyr1, outimg1),(out_pyr2, outimg2)):
    ...
    usr_out = PyramidBlending.collapse(pyr)
...
if __name__ == "__main__":
  ...
  sourcefolder = os.path.abspath(os.path.join(os.curdir, 'images', 'source','sample'))
